...
import com.demo.Attendance.mapper.CustomPageMapper;
...
public class CourseController {
    ...
    public ResponseEntity<CustomPageDto<CourseResponseDto>> getCourseByName(
            @RequestParam String courseName,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "100") int size) {

        Pageable pageable = PageRequest.of(page, size);
        Page<CourseResponseDto> courseResponseDtoPage = courseService.searchByCourseName(courseName, pageable);
        CustomPageDto<CourseResponseDto> customPageDto = CustomPageMapper.customPageDto(courseResponseDtoPage);
        return new ResponseEntity<>(customPageDto,HttpStatus.ACCEPTED);
    }
